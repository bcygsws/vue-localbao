...
- 新闻详情页=详情内容区+评论区（用户评论框【提交评论】+评论列表展示+【加载更多】）

#### 【生活趣图】-图片展示页

- 六宫格【图片分享】按钮。作用：展示趣图列表信息
- 图片列表包括：顶部滑动条和图片列表
...
- 点击任何一张图片->图片详情页
  ...
  - 图片详情页：包括详情内容、缩略图、评论列表（复用抽象出来的 Comment.vue 作为子组件，给子组件传一个分类 id）。难点：缩略图部分使用 vue-preview 包实现。a.vue-preview 包的引入、注册。b.请求缩略图数组数据后，需要对数组遍历(ForEach),同时对 width、heigth、src 属性进行处理，转化成 w、h、msrc,这样 vue-preview 渲染时才能识别。c.同时，缩略图样式需要使用深度选择器/deep/
